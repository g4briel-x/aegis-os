<#
.SYNOPSIS
Shows one persisted Aegis OS execution session.

.ROLE
Delegates persisted execution session lookup to the
...
if ([string]::IsNullOrWhiteSpace($Argument)) {
    Write-Host `
        'Usage: .\cli\aegis.ps1 runtime:session-show <session-id-or-workspace-id>' `
