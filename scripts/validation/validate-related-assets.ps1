...
Exposes Test-AegisRelatedAssets so it can be dot-sourced by an orchestrator
(e.g. validate-registry.ps1) and reused against an already-scanned file list.

.USAGE
pwsh -ExecutionPolicy Bypass -File scripts\validation\validate-related-assets.ps1
#>
...
function Test-AegisRelatedAssets {
    param(
        [Parameter(Mandatory)]
    ...
    $relatedRefs = [System.Collections.Generic.List[object]]::new()
    ...
    foreach ($file in $YamlFiles) {
        $lines = $fileContents[$file.FullName]
        $insideEntries = $false
        ...
        foreach ($line in $lines) {
            ...
            if ($insideRelatedAssets -and $line -match "^\s{6,}-\s*id:\s*(.+)\s*$") {
                ...
                if (-not [string]::IsNullOrWhiteSpace($relatedId)) {
                    $relatedRefs.Add([PSCustomObject]@{
                        File = $file.FullName
                        Line = $lineNumber
                        Id   = $relatedId
                    })
    ...
    foreach ($ref in $relatedRefs) {
        if ($declaredIds.Contains($ref.Id)) {
            ...
            $warnings.Add("$($ref.File):$($ref.Line) -> unknown related asset: $($ref.Id)")
...
if ($MyInvocation.InvocationName -ne '.') {
    ...
    $warnings = Test-AegisRelatedAssets -YamlFiles $yamlFiles
